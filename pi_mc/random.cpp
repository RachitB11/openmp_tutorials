...
// Since it is a multi threaded system and this is a global variable that is
// constantly being modified there is bound to be a race condition. So we need
// a private copy of this variable in each thread.
...
//
// If we choose a single seed and randomly jump through the sequence we get more
// unevenly sampled distribution the more threads there are because the pseudo
// random sequences overlap. So we use this leapfrog method using powers of the
// multiplier so that the pseudo random number sequences don't overlap and we
